				...
				u = {
					...
					secondary: function (d) {
						var f = d === 'right' ? 'left' : 'top',
							h = p[f];
						return (
							p[d] > c[d] && !t.escapeWithReference && (h = Math.min(p[f], c[d] - (d === 'right' ? p.width : p.height))),
							Ge({}, f, h)
						);
					},
				};
					...
					(Oo = le({
						'../../node_modules/prop-types/factoryWithTypeCheckers.js'(e, t) {
							...
							t.exports = function (l, p) {
								...
								function m(E) {
									function x(W, D, L, z, M, F) {
										...
										if (K !== E) {
											var Y = Re(G);
											return new v(
												'Invalid ' + z + ' `' + M + '` of type ' + ('`' + Y + '` supplied to `' + L + '`, expected ') + ('`' + E + '`.'),
												{ expectedType: E },
											);
										}
										return null;
								...
								function B() {
									function E(x, W, D, L, z) {
										...
										if (!l(M)) {
											var F = he(M);
											return new v(
												'Invalid ' + L + ' `' + z + '` of type ' + ('`' + F + '` supplied to `' + D + '`, expected a single ReactElement.'),
											);
										}
										return null;
									}
								...
								function te(E) {
									function x(W, D, L, z, M) {
										...
										var F = W[D],
											G = he(F);
										if (G !== 'object')
											return new v('Invalid ' + z + ' `' + M + '` of type ' + ('`' + G + '` supplied to `' + L + '`, expected an object.'));
										for (var K in F)
											if (i(F, K)) {
												var Y = E(F, K, L, z, M + '.' + K, o);
												if (Y instanceof Error) return Y;
								...
								function Ve() {
									function E(x, W, D, L, z) {
										return Te(x[W]) ? null : new v('Invalid ' + L + ' `' + z + '` supplied to ' + ('`' + D + '`, expected a ReactNode.'));
									}
									return T(E);
								}
								...
								function Be(E) {
									function x(W, D, L, z, M) {
										var F = W[D],
											G = he(F);
										if (G !== 'object')
											return new v('Invalid ' + z + ' `' + M + '` of type `' + G + '` ' + ('supplied to `' + L + '`, expected `object`.'));
										for (var K in E) {
											var Y = E[K];
						...
						componentDidUpdate(e) {
							...
							(h('lifecycle', k.INIT, k.READY) && d.update({ lifecycle: pr(u) || N ? k.TOOLTIP : k.BEACON }),
								...
								f('lifecycle', k.TOOLTIP) &&
									...
									this.tooltip && ((this.scope = new Ga(this.tooltip, { selector: '[data-action=primary]' })), this.scope.setFocus())),
